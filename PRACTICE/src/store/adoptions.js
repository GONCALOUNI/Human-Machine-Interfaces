...
export const useAdoptionsStore = defineStore({
  ...
  getters: {
    ...
    async getMyadoptionsDB(id) {
            try {
                const response = await fetch(`http://daw.deei.fct.ualg.pt/~a76967/PRACTICE/api/adoptions.php?petlover_id=${id}`)
    ...
    async newAdoptionDB(newAdoption) {         
          try {
              const response = await fetch('http://daw.deei.fct.ualg.pt/~a76967/PRACTICE/api/adoptions.php', {
                  method: 'POST',
