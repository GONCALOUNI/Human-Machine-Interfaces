      ...
      <div class="content-container py-5 h-100 d-flex">
		<form @submit.prevent="postBlog">
		  ...
		  <div class="button-container mt-3">
		    <button type="submit" class="btn btn-green btn-lg" name="go">Go</button>
...
export default { 
  ...
  methods:
  {
    async postBlog()
    {
      ...
      try
      {
        const postSuccess = await this.micropostsStore.addMicropostDB({
        ...
        if(postSuccess) {this.$router.push({path: `/message/${3}`});}
      }
      catch(error)
      {
      }
