	...
	<div>
	  ...
	  <div class="container position-relative shadow" style="margin-top: -20px; margin-bottom: 8px;">
		<div class="content-container py-5 h-100 d-flex">
		  <form @submit.prevent="commentAction">
			  <textarea v-model="comment.content" placeholder="Type your comment here:" rows="5" style="resize: none !important; width: 420px; height: 245px; margin-top: 25px; margin-left: 170px;"></textarea>
			  <div class="button-container mt-3">
			    <button type="submit" class="btn btn-green btn-lg" name="comment">Comment</button>
...
export default {
  ...
  methods:
  {
    ...
    async commentAction()
    {
      ...
      {
        this.$router.push({path:`/message/${10}`});
        return;
      }
      this.submitting = true;
      try
      {
        const commentSuccess = await this.commentsStore.addCommentDB({
          session_id: this.userStore.user.session_id,
          post_id: this.$route.params.id,
          post: this.comment
        });
        if(commentSuccess) {this.$router.push({path:`/message/7`});}
      }
      catch(error)
      {
      }
