...
export const useCommentsStore = defineStore({
    ...
    actions: {
        ...
        async getCommentsDB() {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a76967/LAB8_10/api/comments.php`)
        ...
        async addCommentDB(newComment) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a76967/LAB8_10/api/comments.php?micropost_id=${newComment.post_id}&session_id=${newComment.session_id}`, {
					method: 'POST',
